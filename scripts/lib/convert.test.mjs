...
// --- numbered section headings, and tabbed indents ----------------------

test('a numbered list item becomes a section marker, number and all', () => {
    // Docs writes "1. Straw Theory" as an <ol><li>, with the number
    // generated by the list rather than stored in the text. The body loop
    // only looked at top-level blocks, so Pissing Diptych's two section
    // titles were dropped from the page entirely - absent, not restyled.
    const html = docExport(
        '<ol start="1"><li><span>Straw Theory</span></li></ol>' +
            p('Patton says this about pissing,') +
            '<ol start="2"><li><span>Praxis</span></li></ol>' +
            p('Patton had a thing for pissing') +
            p('3.3.10')
    );
    const { body, date } = convertDocHtml(html, 'Pissing Diptych', new Map());

    assert.match(body, /<span class="stanza">1\. Straw Theory<\/span>/);
    assert.match(body, /<span class="stanza">2\. Praxis<\/span>/);
    assert.equal(date, '3.3.10');
});

test('a list that continues numbering keeps its own start', () => {
    const html = docExport('<ol start="7"><li><span>Seventh</span></li></ol>' + p('1.1.11'));
    const { body } = convertDocHtml(html, 'T', new Map());

    assert.match(body, /<span class="stanza">7\. Seventh<\/span>/);
});

test('an unnumbered list item is a marker without a number', () => {
    const html = docExport('<ul><li><span>Praxis</span></li></ul>' + p('1.1.11'));
    const { body } = convertDocHtml(html, 'T', new Map());

    assert.match(body, /<span class="stanza">Praxis<\/span>/);
});

test('a line tabbed at its first character is indented', () => {
    // Pressing Tab in Docs sets text-indent, not margin-left. Reading
    // only the margin left those lines flush while the Doc showed them
    // stepped in - most of Nostalgia looked wrong for this reason.
    const html = docExport(
        '<p class="ti1"><span>last longer than a day.</span></p>' +
            '<p class="ti2"><span>but once in a blue moon:</span></p>' +
            p('1.1.11'),
        '.ti1{text-indent:36pt}\n.ti2{text-indent:72pt}'
    );
    const { body } = convertDocHtml(html, 'T', new Map());

    assert.match(body, /<span class="indent">last longer than a day\.<\/span>/);
    assert.match(body, /<span class="double-indent">but once in a blue moon:<\/span>/);
});
