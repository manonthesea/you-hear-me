...
// Docs writes a numbered section heading ("1. Straw Theory") as a list,
// with the number generated by the <ol> rather than stored in the text.
// The body loop only ever looked at top-level blocks, so both of Pissing
// Diptych's section titles were dropped from the page entirely - not
// mis-styled, absent.
...
function indentClassFor(props) {
    // text-indent matters as much as margin: pressing Tab at the start of
    // a line in Docs sets a first-line indent, not a margin, and reading
    // only the margin left those lines flush against the left edge while
    // the Doc showed them stepped in.
    const px = Math.max(
        toPx(props['margin-left']),
        toPx(props['padding-left']),
        toPx(props['text-indent'])
    );
...
// Flattens <ol>/<ul> into their items, reconstructing the number Docs
// left to the list to render. An unnumbered list keeps its text alone.
function expandLists($, nodes) {
    const out = [];
    for (const node of nodes) {
        ...
        const start = Number.parseInt($(node).attr('start') ?? '1', 10) || 1;
        $(node)
            .children('li')
            .each((i, li) => {
                out.push({ el: li, marker: ordered ? `${start + i}.` : null });
            });
    }
    return out;
}
...
export function convertDocHtml(html, title, docIdToPath, currentDir = '') {
    ...
    const blocks = expandLists($, $('body').children().toArray());
    ...
    for (const { el, marker } of blocks) {
        const $el = $(el);
        const tag = el.tagName?.toLowerCase();
        if (containerNodes.has(el)) continue; // a footnote body, not a poem line
        if (!BLOCK_TAGS.includes(tag) && tag !== 'li') continue;
        ...
        // A list item is a section marker whatever it says, and carries
        // back the number the <ol> would have rendered.
        ...
        for (const segment of segments) {
            ...
            if (isListItem) {
                lines.push({ type: 'stanza', text: marker ? `${marker} ${text}` : text });
                continue;
            }
