...
- **From GitHub**: Actions tab → "Sync poems" workflow → Run workflow.
  ...
  Commonplace Book (`commonplace.html`) and the Paths (`paths.html`),
  and pushes a commit only if something changed. This stays the way to publish a **Doc edit**: nothing on
